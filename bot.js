		...
		if (command === 'help') {
			...
			if (!args[0]) {
				...
				if (message.member.roles.cache.some(r => r.name.toLowerCase() === 'splash')) {
					message.member.roles.remove(message.guild.roles.cache.some(r => r.name.toLowerCase() == 'splash'));
				}
				else {
					message.member.roles.add(message.guild.roles.cache.some(r => r.name.toLowerCase() == 'splash'));
				}
				...
				if (message.member.roles.cache.some(r => r.name.toLowerCase() === 'update')) {
					message.member.roles.remove(message.guild.roles.cache.some(r => r.name.toLowerCase() == 'update'));
				}
				else {
					message.member.roles.add(message.guild.roles.cache.some(r => r.name.toLowerCase() == 'update'));
				}
				message.channel.send(":white_check_mark: Successfully updated your roles.").then(msg => {msg.delete({timeout:4000})});
			}
			else {
